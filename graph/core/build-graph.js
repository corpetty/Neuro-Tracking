...
export function buildGraph(ctx) {
  ...
  for (const doc of proseDocs) {
    ...
    // LOCAL PATCH: strip fenced blocks and inline code before scanning, so a
    // `[[wikilink]]` written as a code span in a how-to page is not mistaken
    // for a real link.
  ...
  const evidencePreds = new Set(evidencePredicates(ontology));
  const sourcesWithEvidence = new Set(
    edges.filter((e) => evidencePreds.has(e.predicate)).map((e) => e.source),
  );
  for (const n of nodes.values()) {
    if (n.type === (ontology.sourceType || 'Source') && n.file && !sourcesWithEvidence.has(n.id)) {
      warn(`source unread: ${n.id} has a file but no supports/pressureTests evidence yet`);
    }
  }

  // (7) Orphan content: prose files not claimed by any Document. LOCAL PATCH:
  // walk the tree (the Quartz corpus is nested) and compare by relative path,
  // honouring profile.json's orphanIgnore globs.
  if (existsSync(ctx.contentDir)) {
    const claimed = new Set(proseDocs.map((d) => d.file.replace(/^\.\//, '')));
    const ignore = (ctx.profileMeta?.orphanIgnore || []).map((g) => new RegExp(g));
    const walk = (dir, rel = '') => {
      for (const ent of readdirSync(dir, { withFileTypes: true })) {
        const r = rel ? `${rel}/${ent.name}` : ent.name;
        if (ent.isDirectory()) walk(join(dir, ent.name), r);
        else if (ent.name.endsWith('.md') && !claimed.has(r) && !ignore.some((re) => re.test(r)))
          warn(`orphan content: ${r} is not referenced by any ${ontology.documentType || 'Document'}`);
      }
    };
    walk(ctx.contentDir);
  }
