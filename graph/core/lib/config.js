...
export function resolveProfile(argv = process.argv) {
  ...
  return process.env.PROFILE || 'neuro';
}
...
export function loadContext(profileName = resolveProfile()) {
  ...
  // LOCAL PATCH (Neuro-Tracking): an optional profile.json lets a profile point
  // at a corpus that lives outside profiles/ — here, the Quartz content/ tree —
  // instead of duplicating prose into profiles/<name>/content.
  const profileMeta = readJSONIf(join(profileDir, 'profile.json'), {});
  ...
  return {
    ...
    profileDir,
    profileMeta,
    contentDir: profileMeta.contentDir
      ? resolve(profileDir, profileMeta.contentDir)
      : join(profileDir, 'content'),
