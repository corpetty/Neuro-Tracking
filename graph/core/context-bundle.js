...
export function buildBundle(ctx, rawCenter, opts = {}) {
  ...
  function renderSection(sec) {
    switch (sec.kind) {
      ...
      case 'sources': {
        const ss = byType(sec.nodeType || ontology.sourceType || 'Source');
        if (!ss.length) return '';
        const lines = [`## ${sec.title}`];
        for (const s of ss) {
      ...
      case 'editorialFlags': {
        ...
        const rival = sec.rivalPredicate;
        if (rival) {
          for (const c of claims) {
            const rivals = inEdges(c.id, rival);
            if (rivals.length && !evOf(c.id, pos).length)
              flags.push(
                `⚖ ${rivals.length} rival explanation(s) not ruled out for **${labelOf(c)}**: ${rivals
                  .map((e) => labelOf(nodeById.get(e.source)))
                  .join(', ')}`,
              );
          }
        }
        if (['skeleton', 'not-started', 'seed'].includes(centerNode.status))
  ...
  function renderClaim(c, evPreds, dir = 'in') {
    ...
    // LOCAL PATCH: profile-declared extra relations worth carrying with a claim
    // (here: the rival explanations that `confounds` it, and what it would
    // explain). Without this a bundle can present a hypothesis with its
    // supporting evidence while silently dropping its competitors.
    for (const ex of renderSpec.claimExtras || []) {
      const es = ex.dir === 'in' ? inEdges(c.id, ex.predicate) : outEdges(c.id, ex.predicate);
      const labels = es.map((e) => labelOf(nodeById.get(ex.dir === 'in' ? e.source : e.target)));
      if (labels.length) lines.push(`_${ex.label}: ${labels.join(', ')}._`);
    }
    let any = false;
