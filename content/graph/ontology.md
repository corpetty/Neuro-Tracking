...
|---|---|---|---|
| **Note** | `note:` | A page in the garden — a paper summary, a symptom log, a session note, an index. The only type that owns prose on the site. | 16 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `mayExplain` | Mechanism \| Hypothesis \| Intervention → Symptom \| Finding | Candidate explanation for an experience or result. HYPOTHETICAL by construction — never a diagnosis. | 18 |
...
| `flagsOpenQuestion` | Note \| Hypothesis \| Session \| Finding \| Symptom → Question | Flags an open question the node rests on. | 13 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `wikiLinks` | Note → Note \| Session \| Symptom \| Finding \| Instrument \| Intervention \| Mechanism \| Hypothesis \| Question \| Paper \| Tension \| Author | Wiki-style link parsed from prose. Navigation only — kept out of the argument graph. | 45 |
