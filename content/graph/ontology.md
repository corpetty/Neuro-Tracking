...
|---|---|---|---|
| **Note** | `note:` | A page in the garden — a paper summary, a symptom log, a session note, an index. The only type that owns prose on the site. | 19 |
...
| **Instrument** | `instrument:` | A measurement tool: a screener, a questionnaire, a cognitive task, a lab panel, a wearable metric. | 10 |
...
| **Mechanism** | `mechanism:` | A named cognitive/neural construct the notebook reuses: response inhibition, statistical learning, mind-wandering, executive function. | 16 |
| **Hypothesis** | `hypothesis:` | A load-bearing assertion the notebook is trying to defend or falsify. Never a diagnosis — always something that can be pressure-tested. | 21 |
| **Question** | `question:` | An open problem the notebook is working on; can gate a Note, a Hypothesis, or a next Session. | 4 |
| **Paper** | `paper:` | An external work cited: journal article, preprint, book, clinical guideline. | 58 |
| **Author** | `author:` | Who produced a Paper. | 58 |
| **Tension** | `tension:` | A competing-explanation or trade-off held open on purpose — e.g. ADHD vs. anxiety as the source of the same symptom. Reified so it cannot quietly resolve itself. | 5 |
...
|---|---|---|---|
| `partOf` | Note → Note | Note belongs to a parent Note (section index). | 18 |
| `covers` | Note → Symptom \| Finding \| Session \| Instrument \| Intervention \| Mechanism \| Hypothesis \| Question \| Tension | This note is where the thing is written up. | 12 |
| `measures` | Instrument → Symptom \| Mechanism | Instrument is designed to measure this symptom domain or construct. | 19 |
| `hasStatus` | Note \| Hypothesis \| Question \| Intervention → Status | Node has this lifecycle status. | 40 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `mayExplain` | Mechanism \| Hypothesis \| Intervention → Symptom \| Finding | Candidate explanation for an experience or result. HYPOTHETICAL by construction — never a diagnosis. | 20 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `documentedIn` | Session \| Finding \| Symptom \| Intervention \| Tension → Note | The case fact is written up in this note. | 13 |
| `definedIn` | Mechanism → Note | Mechanism has its primary development in this note. | 16 |
| `cites` | Note → Paper | Note cites a paper (a deliberate act, never inferred from prose). | 60 |
| `authoredBy` | Paper → Author | Paper authored by Author. | 78 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `supports` | Paper → Hypothesis \| Mechanism · carries evidence | Paper supports this hypothesis/mechanism. The edge carries the verbatim quote + page locator + confidence + rationale, so a fact and its justification travel together. | 55 |
...
| `confounds` | Mechanism \| Symptom \| Intervention → Hypothesis | A rival explanation that must be ruled out before the hypothesis can stand. The honesty predicate of this ontology. | 4 |
| `contradicts` | Hypothesis \| Mechanism \| Finding → Hypothesis \| Mechanism \| Finding (undirected) | Direct contradiction. | 0 |
| `tensionWith` | Hypothesis \| Mechanism \| Symptom \| Finding \| Tension → Hypothesis \| Mechanism \| Symptom \| Finding (undirected) | In unresolved tension with (both endpoints stay live). | 9 |
...
| `flagsOpenQuestion` | Note \| Hypothesis \| Session \| Finding \| Symptom → Question | Flags an open question the node rests on. | 16 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `argues` | Note → Hypothesis | Note argues this hypothesis. | 21 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `wikiLinks` | Note → Note \| Session \| Symptom \| Finding \| Instrument \| Intervention \| Mechanism \| Hypothesis \| Question \| Paper \| Tension \| Author | Wiki-style link parsed from prose. Navigation only — kept out of the argument graph. | 53 |
...
_207 nodes · 464 edges · ontology v0.1.0._
