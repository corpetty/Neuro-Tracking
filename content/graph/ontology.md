...
|---|---|---|---|
| **Note** | `note:` | A page in the garden — a paper summary, a symptom log, a session note, an index. The only type that owns prose on the site. | 30 |
...
| **Mechanism** | `mechanism:` | A named cognitive/neural construct the notebook reuses: response inhibition, statistical learning, mind-wandering, executive function. | 23 |
| **Hypothesis** | `hypothesis:` | A load-bearing assertion the notebook is trying to defend or falsify. Never a diagnosis — always something that can be pressure-tested. | 43 |
| **Question** | `question:` | An open problem the notebook is working on; can gate a Note, a Hypothesis, or a next Session. | 4 |
| **Paper** | `paper:` | An external work cited: journal article, preprint, book, clinical guideline. | 93 |
| **Author** | `author:` | Who produced a Paper. | 83 |
...
|---|---|---|---|
| `partOf` | Note → Note | Note belongs to a parent Note (section index). | 29 |
| `covers` | Note → Symptom \| Finding \| Session \| Instrument \| Intervention \| Mechanism \| Hypothesis \| Question \| Tension | This note is where the thing is written up. | 54 |
| `measures` | Instrument → Symptom \| Mechanism | Instrument is designed to measure this symptom domain or construct. | 23 |
| `hasStatus` | Note \| Hypothesis \| Question \| Intervention → Status | Node has this lifecycle status. | 76 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `mayExplain` | Mechanism \| Hypothesis \| Intervention → Symptom \| Finding | Candidate explanation for an experience or result. HYPOTHETICAL by construction — never a diagnosis. | 40 |
...
| `definedIn` | Mechanism → Note | Mechanism has its primary development in this note. | 23 |
| `cites` | Note → Paper | Note cites a paper (a deliberate act, never inferred from prose). | 102 |
| `authoredBy` | Paper → Author | Paper authored by Author. | 116 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `supports` | Paper → Hypothesis \| Mechanism · carries evidence | Paper supports this hypothesis/mechanism. The edge carries the verbatim quote + page locator + confidence + rationale, so a fact and its justification travel together. | 114 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `argues` | Note → Hypothesis | Note argues this hypothesis. | 49 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `wikiLinks` | Note → Note \| Session \| Symptom \| Finding \| Instrument \| Intervention \| Mechanism \| Hypothesis \| Question \| Paper \| Tension \| Author | Wiki-style link parsed from prose. Navigation only — kept out of the argument graph. | 115 |
...
_321 nodes · 854 edges · ontology v0.1.0._
