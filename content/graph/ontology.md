...
|---|---|---|---|
| **Note** | `note:` | A page in the garden — a paper summary, a symptom log, a session note, an index. The only type that owns prose on the site. | 24 |
...
| **Hypothesis** | `hypothesis:` | A load-bearing assertion the notebook is trying to defend or falsify. Never a diagnosis — always something that can be pressure-tested. | 30 |
| **Question** | `question:` | An open problem the notebook is working on; can gate a Note, a Hypothesis, or a next Session. | 4 |
| **Paper** | `paper:` | An external work cited: journal article, preprint, book, clinical guideline. | 78 |
| **Author** | `author:` | Who produced a Paper. | 70 |
...
|---|---|---|---|
| `partOf` | Note → Note | Note belongs to a parent Note (section index). | 23 |
| `covers` | Note → Symptom \| Finding \| Session \| Instrument \| Intervention \| Mechanism \| Hypothesis \| Question \| Tension | This note is where the thing is written up. | 33 |
| `measures` | Instrument → Symptom \| Mechanism | Instrument is designed to measure this symptom domain or construct. | 23 |
| `hasStatus` | Note \| Hypothesis \| Question \| Intervention → Status | Node has this lifecycle status. | 57 |
...
| `cites` | Note → Paper | Note cites a paper (a deliberate act, never inferred from prose). | 80 |
| `authoredBy` | Paper → Author | Paper authored by Author. | 101 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `argues` | Note → Hypothesis | Note argues this hypothesis. | 32 |
...
| Predicate | Shape | Means | Count |
|---|---|---|---|
| `wikiLinks` | Note → Note \| Session \| Symptom \| Finding \| Instrument \| Intervention \| Mechanism \| Hypothesis \| Question \| Paper \| Tension \| Author | Wiki-style link parsed from prose. Navigation only — kept out of the argument graph. | 74 |
